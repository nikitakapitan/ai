from ai.activation_functions import Sigmoid, Linear
from ai.target_functions import QuadraticCost, CrossEntropyCost
from ai import vizualisation as vizual
import random
import numpy as np
from matplotlib import pyplot as plt

from ai import vizualisation
    ...
    def fit(self, X, y, epochs=1, batch_size=1, learning_rate=1, eps=1e-6, test_data=None, valid_data=None):
        ...
        if test_data:
            print(f'Training on {n} examples during {epochs} epochs. {batch_size} elements in batch \n \
                  Initial random accuracy: {self.evaluate(*test_data)}% classified.\n \
                  Cost function : {self.cost.function(self.predict(test_data[0]), test_data[1])} to be minimized')


        bar = progressbar.ProgressBar(maxval=epochs, widgets=[progressbar.Bar(marker='#', left='[', right=']'), ' ',
        ...
        print(f'Final network state:\n \ {self.evaluate(*test_data)}% classified.\n \ Cost function: {self.cost.function(self.predict(test_data[0]), test_data[1])}')
...
#         delta = j_deriv * self.activation.derivative(zs[-1]) # (K, B)
    ...
    def vizualise(self):
        ...
        vizualisation.target(self.debug['target_train'], self.debug['target_test'], self.debug['target_valid'], ax=ax1)
        vizualisation.misclassification(self.debug['misclass_train'], self.debug['misclass_test'], self.debug['misclass_valid'], ax=ax2)
